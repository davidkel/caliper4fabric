...
function injectCustomVersion(packageJsonPath, customVersion) {
    ...
    for (let dep of Object.keys(packageObject.dependencies)) {
        if (dep.startsWith('@davidkel/caliper-')) {
        ...
        for (let pkg of packages) {
            ...
            try {
                ...
                for (let i = 0; i < retries; i++) {
                    log(`Publishing @davidkel/${pkg}@${packageVersion} with tag "${tag}". Attempt ${i+1}/${retries}:`);
                    try {
                        await utils.invokeCommand(scriptPath, [], envs, packageDir);
                        log(`Published package @davidkel/${pkg}@${packageVersion}\n`);
                        published = true;
                        break;
                    } catch (error) {
                        log(`Failed to publish package @davidkel/${pkg}@${packageVersion} (attempt ${i+1}/${retries})`);
                        log(error);
                    }
                }

                if (!published) {
                    log(`Aborting, could not publish package @davidkel/${pkg}@${packageVersion}`);
                    process.exit(1);
                }
            } catch (err) {
                log(`Aborting, could not publish package @davidkel/${pkg}@${packageVersion}`);
